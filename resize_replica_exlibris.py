...
def get_args():
    ...
    parser.add_argument('-s', '--sourceibox', nargs=1, required=True, help='Source ibox name or FQDN', dest='source_box_name_or_fqdn', type=str)
    parser.add_argument('-t', '--targetibox', nargs=1, required=True, help='Target ibox name or FQDN', dest='target_box_name_or_fqdn', type=str)
    parser.add_argument('-m', '--maphost', nargs=1, required=True, help='The host name to map target system volumes to (as shown in taget system hosts tab)', dest='map_host', type=str)
...
if __name__ == '__main__':
    args = get_args()
    source_box_name_or_fqdn=args.source_box_name_or_fqdn[0]
    target_box_name_or_fqdn=args.target_box_name_or_fqdn[0]
    map_host=args.map_host[0]
